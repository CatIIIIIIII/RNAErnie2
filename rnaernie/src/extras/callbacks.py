        ...
        if args.should_save:
            fix_valuehead_checkpoint(
                ...
                output_dir=os.path.join(
                    args.output_dir, "{}-{}".format(PREFIX_CHECKPOINT_DIR, state.global_step)),
...
class LogCallback(TrainerCallback):
    def __init__(self, output_dir: str) -> None:
        ...
        self.webui_mode = os.environ.get(
            "LLAMABOARD_ENABLED", "0").lower() in ["true", "1"]
        if self.webui_mode:
    ...
    def on_log(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        ...
        logs = dict(
            ...
            percentage=round(self.cur_steps / self.max_steps *
                             100, 2) if self.max_steps != 0 else 100,
        ...
        if has_length(eval_dataloader):
            ...
            if self.cur_steps % 5 == 0 and self.thread_pool is not None:
                logs = dict(
                    ...
                    total_steps=self.max_steps,
                    percentage=round(
                        self.cur_steps / self.max_steps * 100, 2) if self.max_steps != 0 else 100,
                    elapsed_time=self.elapsed_time,
                    remaining_time=self.remaining_time,
                )
                self.thread_pool.submit(self._write_log, args.output_dir, logs)
